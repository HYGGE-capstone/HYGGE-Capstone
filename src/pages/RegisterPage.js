...
function RegisterPage() {
  ...
  const checkIdCanUsable = async (e) => {
    e.preventDefault();
    await axios
      .get(`http://43.201.179.98:80/api/auth/signup/loginId/${userId}`) //임시
      .then((resp) => {
        alert("사용 가능한 아이디입니다.");
        setidCanUsable(true);
      })
  ...
  const emailVerCheck = async (e) => {
    await axios
      .post(
        `http://43.201.179.98:80/api/auth/signup/email/auth?to=${userEmail}`
      ) //임시
      .then((resp) => {
  ...
  const onSubmit = async () => {
    ...
    await axios
      .post(`http://43.201.179.98:80/api/auth/signup`, user) //임시
      .then((resp) => {
        alert("회원가입 완료!");
        navigate("/login");
      })
